 ...
 */
function initialize(){
	d3.request("../data/chain-geo.json")		// 地图数据
		...
		.get()
	;
	d3.request("../data/province.json")			// 省份数据
	    .mimeType("application/json")
	    .response(function(xhr) {
 ...
 */
function init_root() {
	var root_node = GD_data[G_time.index]['target'];
	add_node(root_node);
	add_link(GD_data[G_time.index]['link'])
	restart();
	G_time.index++;
...
function dynamic_process(){
	G_time.loop = setInterval(function(){
		...
		while (count > 0) {
			try {
				var s = GD_data[G_time.index]['source'];
				if (GD_wbids.indexOf(s.id) == -1) {
					add_node(s);
				}
				var t = GD_data[G_time.index]['target'];
