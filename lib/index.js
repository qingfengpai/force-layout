	...
	if (event.keyCode == 13) {
		...
		for (var i = GD_nodes.length - 1; i >= 0; i--) {
			if (GD_nodes[i]['uid'] == value) {
				execute_mouseover(GD_nodes[i]['ancestors'], GD_nodes[i]['uid']);
				searched = true;
				break;
			}
		}
 ...
 * 暂停按钮
 */
var E_stop_btn = document.getElementById("play");
E_stop_btn.onclick = toggle_timer;
function toggle_timer(){
	var status = E_stop_btn.dataset.status;
	// 点击开始展示
	if (status == "stop") {
		E_stop_btn.classList.add("hide");
		E_stop_btn.dataset.status = "start";
		dynamic_ordered();
	}
	// 点击显示暂停
	else {
		E_stop_btn.classList.remove("hide");
...
function update_curr_node_info(uid) {
	if (!uid) {
		uid = G_curr.node.getAttribute('uid');
	}
	var d = GD_total_users[uid];
	if (d.sex == "m") {
		E_sex.innerText = "男";
	} else if (d.sex == "f") {
		E_sex.innerText = "女";
	...
	E_depth.innerText = parseInt(G_curr.node.getAttribute('depth')) + 1;
}
...
function init_root() {
	...
	// var img = GD_total_users[root_node.uid].avatar;
...
function add_random_node(parent_node){
	...
	var depth = parent_node.dataset.ancestors.split('-').length;
	var new_node = {
		"time": new Date(),
