...
function update_line_item() {
	G_x_scale = d3.scaleTime()
		.domain([new Date(1483452853), new Date(G_time.end)])
...
function dynamic_ordered(){
	G_time.loop = setInterval(function(){
		...
		while (true) {
			...
			// var span = item['time'] - G_time.end;
			// if (span < 60) {
			// 	G_time.end += 60;
			// 	GD_chart.push({'x': G_chart.end, 'y': 0});
			// 	break;
			// }
			...
			if (gap > G_time.gap) {
				...
				GD_chart.push({'x': item['time'], 'y': count});
